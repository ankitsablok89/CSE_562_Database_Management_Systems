...
public class ProjectTableOperation {
	...
	public static Table projectTable(Table tableToProject,ArrayList<String> selectList,boolean flag) throws IOException, ParseException{
	
		
		// this is the file object for the projected table
		File newTableFile = new File(tableToProject.tableDataDirectoryPath.getAbsolutePath() + System.getProperty("file.separator") + tableToProject.tableName+"ProjectedTable.tbl");
		BufferedWriter bw = new BufferedWriter(new FileWriter(newTableFile) , 32768);
	
		
		Table ProjectedTable=new Table(tableToProject.tableName+"ProjectedTable.tbl",selectList.size(),newTableFile, tableToProject.tableDataDirectoryPath)	;
		...
		for(String str:selectList)
		{
			...
			if(str.contains(" AS ")||str.contains(" as "))
			{
			    ...
			    ColDataType col=new ColDataType();
				colDescTypePair.setColumnName(temp[temp.length-1]);
				String type=tableToProject.columnDescriptionList.get(tableToProject.columnIndexMap.get(temp[0])).getColDataType().getDataType();
...
for(String str:selectList)
{
	...
	else if(str.contains("SUM")||str.contains("AVG")||str.contains("sum")||
			   str.contains("avg")||str.contains("min")||str.contains("max")
			   ||str.contains("COUNT")|| str.contains("count")||str.contains("MIN")
			   ||str.contains("MAX")){
	    ColumnDefinition colDescTypePair=new ColumnDefinition();
	    ColDataType col=new ColDataType();
	...
	else
	{
		...
		String type=tableToProject.columnDescriptionList.get(tableToProject.columnIndexMap.get(str)).getColDataType().getDataType();
